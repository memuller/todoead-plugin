				...
				if(isset($name)){
					...
					foreach ($class::$fields as $field => $options) {
						if( 'date' == $options['type'] ){
							...
							wp_enqueue_script('datepicker', $base::url('js/admin/utils/datepicker.js'), array('jquery-datepick-br'));
						}
						if( 'geo' == $options['type']){
							...
							wp_enqueue_script('geo-field', $base::url('js/admin/utils/geo-field.js'), array('jquery', 'gmaps-api'));
						}
					}
				}
			});
